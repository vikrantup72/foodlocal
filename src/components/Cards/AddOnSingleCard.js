...
export default function AddOnSingleCard({ navigation,selector,item,onSelect,onSelectIndex,index,addressListApi }) {
    ...
    const deleteAddressApi = ()=>{
        ...
        .catch(function(error) {
          console.log('deleteAddressApi',error);
          alert('Error')
          setDeleteAddressLoader(false)
      ...
      const editAddressApi = ()=>{
        ...
        .catch(function(error) {
          console.log('editAddressApiApi',error);
          alert('Error')
          setEditAddressLoader(false)
    ...
    return (
      ...
      <View style={{flex:1, flexDirection:'row',alignItems:'center',justifyContent:'center',paddingHorizontal:10,marginVertical:5}}>
      ...
      <TouchableOpacity style={{
          ...
          shadowRadius: 20,
           backgroundColor: "white", borderRadius: 10, paddingVertical:10, width: '100%',alignSelf:'center',borderWidth:1,borderColor:'#9c3825'
      }} onPress={()=>{
          onSelect(item.id);
      onSelectIndex(index);
      }}>
...
<Text style={{fontFamily: FontFamily.ExpoArabicBook, fontSize: 15, color: "#972C26" }}>{item.name}</Text>
</View>
<View style={{marginLeft: 30, marginVertical:10,backgroundColor:"transparent",alignItems:"center",justifyContent:"center",flex:1}}>
<Text style={{fontFamily: FontFamily.ExpoArabicBook, fontSize: 15, color: "#972C26" }}>{item.price} {t('SAR')}</Text>
</View>
